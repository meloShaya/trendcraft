...
import { Calendar, Flame, Target, TrendingUp, Award, Clock } from 'lucide-react';
...
const PostingStreak: React.FC = () => {
  const { token } = useAuth();
  const [streakData, setStreakData] = useState<PostingStreakData | null>(null);
  ...
  if (!streakData) {
    return (
      <div className="text-center py-8">
        <p className="text-gray-500 dark:text-gray-400">Unable to load streak data</p>
      </div>
    );
  }
  ...
  const getIntensityColor = (level: number) => {
    switch (level) {
      ...
      case 1: return 'bg-purple-200 dark:bg-purple-900/40 border-purple-300 dark:border-purple-700';
      case 2: return 'bg-purple-400 dark:bg-purple-700 border-purple-500 dark:border-purple-600';
      case 3: return 'bg-purple-600 dark:bg-purple-600 border-purple-700 dark:border-purple-500';
      case 4: return 'bg-purple-800 dark:bg-purple-500 border-purple-900 dark:border-purple-400';
    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-4 sm:p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white">Posting Streak</h3>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div className="text-center p-4 bg-gradient-to-br from-purple-50 to-pink-50 dark:from-purple-900/20 dark:to-pink-900/20 rounded-lg border border-purple-200 dark:border-purple-800">
          ...
          <div className="text-xs text-purple-600 dark:text-purple-400">Current Streak</div>
        </div>
        ...
        <div className="text-center p-4 bg-gradient-to-br from-blue-50 to-cyan-50 dark:from-blue-900/20 dark:to-cyan-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
          ...
          <div className="text-xs text-blue-600 dark:text-blue-400">Best Streak</div>
        </div>
        ...
        <div className="text-center p-4 bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-900/20 dark:to-emerald-900/20 rounded-lg border border-green-200 dark:border-green-800">
          ...
          <div className="text-2xl font-bold text-green-700 dark:text-green-300">
            {streakData.streakData.filter(d => d.posted).length}
          </div>
          <div className="text-xs text-green-600 dark:text-green-400">Days Posted</div>
        </div>
        ...
        <div className="text-center p-4 bg-gradient-to-br from-orange-50 to-red-50 dark:from-orange-900/20 dark:to-red-900/20 rounded-lg border border-orange-200 dark:border-orange-800">
          ...
          <div className="text-2xl font-bold text-orange-700 dark:text-orange-300">
            {streakData.streakData.reduce((sum, d) => sum + d.postCount, 0)}
          </div>
          <div className="text-xs text-orange-600 dark:text-orange-400">Total Posts</div>
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-7 gap-1">
          ...
          {calendarDays.map(day => {
            ...
            const intensityLevel = getIntensityLevel(postingData?.postCount || 0);
            const isToday = isSameDay(day, new Date());
            
            return (
              <div
                key={format(day, 'yyyy-MM-dd')}
                className={`aspect-square rounded-sm border-2 flex items-center justify-center text-xs font-medium transition-all hover:scale-110 cursor-pointer ${
                  getIntensityColor(intensityLevel)
                } ${
                  isToday ? 'ring-2 ring-purple-500 ring-offset-1 dark:ring-offset-gray-800' : ''
                }`}
                title={`${format(day, 'MMM d, yyyy')} - ${postingData?.postCount || 0} posts`}
              >
                <span className={`${
                  intensityLevel > 2 ? 'text-white' : 'text-gray-700 dark:text-gray-300'
                }`}>
                  {format(day, 'd')}
                </span>
              </div>
          ...
          <div>
            ...
            {streakData.lastPostDate && (
              <p className="text-xs text-purple-700 dark:text-purple-300 mt-1">
                Last post: {format(parseISO(streakData.lastPostDate), 'MMM d, yyyy')}
              </p>
            )}
          </div>
        </div>
