...
const PlatformConnections: React.FC<PlatformConnectionsProps> = ({ onConnect, onDisconnect }) => {
  ...
  const handleConnect = async (platformId: string) => {
    try {
      ...
      // In a real implementation, you would listen for the OAuth callback
  ...
  const handleDisconnect = (platformId: string) => {
    setPlatforms(prev => prev.map(platform => 
      platform.id === platformId 
        ? { ...platform, connected: false, username: undefined, followers: undefined }
        : platform
    ));
          ...
          >
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center space-x-3">
                <div className={`w-10 h-10 rounded-lg ${platform.color} flex items-center justify-center text-lg font-bold`}>
                  {platform.icon}
                </div>
                <div>
        ...
        <div className="flex items-start space-x-3">
          ...
          <div>
            ...
            <p className="text-sm text-blue-700 dark:text-blue-300 mt-1">
              Connect your social media accounts to publish content directly from TrendCraft. 
