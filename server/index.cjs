// server/index.cjs - FINAL WORKING VERSION
...
// The Correct Way to Import 'apify' in this environment
...
// --- ALL YOUR ROUTES AND HELPER FUNCTIONS GO HERE ---
// (I have pasted them all back in for you below)
...
const fetchTrendsFromApify = async (platform = 'twitter') => {
  try {
    ...
    if (platform === 'twitter' || platform === 'x') {
      ...
      input = { location: 'Worldwide', maxTrends: 20 };
    } else {
      actorId = process.env.SOCIAL_INSIGHT_ACTOR_ID;
      input = { platform: platform, maxResults: 20, sortBy: 'trending' };
    }
    const run = await apifyClient.actor(actorId).call(input, { timeout: 60000, memory: 256 });
...
// ... all your other functions and routes like generateContentWithAI, auth, etc. ...
...
// Auth, Trends, Content, Posts, Analytics, User, etc.
