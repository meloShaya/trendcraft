	...
	return apifyData.map((item, index) => {
		...
		return {
			...
			demographics: {
				...
				interests: item.demographics?.interests || [
					"Technology",
					"Social Media",
					"Trends",
				],
	...
	try {
		...
		return [
			...
			},
			{
				...
				demographics: {
					age: "28-45",
					interests: ["Environment", "Technology", "Sustainability"],
				},
			},
		];
	}
...
`;
		...
		const toneTemplates = {
			professional: [
				`Exploring the future of ${topic}. Key insights and implications for ${
					targetAudience || "professionals"
				}.`,
				`${topic} is transforming industries. Here's what ${
					targetAudience || "leaders"
				} need to know.`,
				`Breaking down ${topic}: Strategic considerations for ${
...
app.get("/api/trends", authenticateToken, async (req, res) => {
	try {
		...
		console.log(
			`Fetching trends - Platform: ${platform}, Category: ${category}, Limit: ${limit}`
		);
...
app.post("/api/content/generate", authenticateToken, async (req, res) => {
	try {
		const { topic, platform, tone, includeHashtags, targetAudience } =
			req.body;
...
app.listen(PORT, () => {
	...
	console.log(
		`🕷️ Apify: ${
			process.env.APIFY_API_TOKEN ? "Configured" : "Not configured"
		}`
	);
});
